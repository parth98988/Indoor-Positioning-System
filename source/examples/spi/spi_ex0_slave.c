...
static uint8 rxBufferSlave[8];
static uint8 txBufferMaster[8];
...
int main(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
      ...
      U0DBUF = txBufferMaster[i];
        ...
        while(!(U0CSR & U0CSR_RX_BYTE)){asm("NOP");}
    ...
    //P0SEL |= ( P0SEL & ~BIT5) |( P0SEL & ~BIT4) | ( P0SEL & ~BIT3) | ( P0SEL & ~BIT2);/
    P0SEL &= ~BIT2;
    P0INP |= 0x02;
